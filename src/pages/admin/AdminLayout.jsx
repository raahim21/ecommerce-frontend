...
const AdminLayout = ({ children }) => {
  ...
  return (
    <div className={`min-h-screen ${theme.pageBg} flex flex-col lg:flex-row`}>
      {/* Mobile Backdrop */}
      {mobileMenuOpen && (
        <div
          className="fixed inset-0 bg-black bg-opacity-50 z-40 lg:hidden"
      ...
      {/* Sidebar - FULL HEIGHT on all screens */}
      <aside
        className={`
          ${mobileMenuOpen ? 'translate-x-0' : '-translate-x-full'}
          lg:translate-x-0
          fixed lg:relative
          top-0 left-0
          w-72 ${theme.cardBg} border-r ${theme.border}
          flex flex-col
          z-50
      ...
      >
        ...
        <div className="p-6 border-t border-gray-200 dark:border-gray-700">
          <button
            onClick={logout}
            className="w-full flex items-center justify-center gap-2 py-3 px-4 rounded-lg text-red-600 font-medium hover:bg-red-50 dark:hover:bg-red-900/20 transition"
          >
            <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17 16l4-4m0 0l-4-4m4 4H7m6 4v1a3 3 0 01-3 3H6a3 3 0 01-3-3V7a3 3 0 013-3h4a3 3 0 013 3v1" />
            </svg>
      ...
      <div className="flex-1 flex flex-col">
        ...
        <button
          ...
          className={`
            ...
            p-3 rounded-full shadow-lg border ${theme.border}
            ${theme.cardBg} hover:scale-110 transition-transform
          `}
        ...
        {/* Page Content */}
        <main className="flex-1 p-6 pt-20 lg:pt-10 lg:p-10 pb-10">
          {children}
        </main>
      </div>
    </div>
